...
TRAIN_PATH = DATA_PROCESSED / "post_match_train_2019_2021.csv"
TEST_PATH  = DATA_PROCESSED / "post_match_test_2023.csv"
MODEL_PATH = MODELS_DIR / "postmatch_logisticregression_model.joblib"

GLOBAL_OUT = DATA_PROCESSED / "shap_global_postmatch.csv"
LOCAL_OUT  = DATA_PROCESSED / "shap_local_examples_postmatch.csv"
CHATBOT_OUT = DATA_PROCESSED / "shap_summary_per_match_postmatch.csv"
...
def main():
    ...
    print("✅ Loaded data & model (POST-MATCH)")
    ...
    # normalize shape: list[class] -> (n_samples, n_features)
    if isinstance(shap_values, list):
        ...
        if sv.ndim == 3 and sv.shape[0] == len(classes):
            shap_values = [ensure_ns_nf(sv[k], n_samples, n_features) for k in range(len(classes))]
        elif sv.ndim == 3 and sv.shape[2] == len(classes):
            shap_values = [ensure_ns_nf(sv[:, :, k], n_samples, n_features) for k in range(len(classes))]
        else:
    ...
    # 2) Predictions table aligned with test rows
    ...
    pd.DataFrame(local_rows).to_csv(LOCAL_OUT, index=False)
    print(f"✅ Saved local SHAP examples: {LOCAL_OUT}")

    # 4) Chatbot summary per match
    summary_rows = []
    for idx in example_idx:
        pred_o = base_pred.loc[idx, "pred_outcome"]
        cls_i = classes.index(pred_o) if pred_o in classes else 0
    ...
    pd.DataFrame(summary_rows).to_csv(CHATBOT_OUT, index=False)
